...
2. Import that data into a table, the structure of which is defined in [`inputs`](./inputs/m311_requests_raw.sql).
...
3. Create the view [`m311_requests_filtered`](./inputs/m311_requests_filtered.sql).
...
4. Create the view [`m311_kmeans_grouped_requests`](./clustering/m311_kmeans_grouped_requests.sql)
    - This clusters the points and adds the group_id (combination of ownergroup and cluster_id within those ownergroups)
5. Create the view [`m311_kmeans_group_boundaries`](./clustering/m311_kmeans_group_boundaries.sql)
